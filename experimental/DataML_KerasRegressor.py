...
def train(learnFile, testFile):
    ...
    estimator = KerasRegressor(build_fn=baseline_model, nb_epoch=dP.epochs,
        batch_size=dP.batch_size, verbose=True)
    
    estimator.fit(A, Cl)
    
    prediction = estimator.predict(A_test)
    print(prediction)
...
def predict(testFile, normFile):
    ...
    if dP.numLabels == 1:
        ...
        print(' ==========================================\n')
        print("  ACN:", str(predValue[0]),"%")
        print("  Water:",str(predValue[1]),"%")
        print("  AcetoNL:",str((predValue[1]/0.5)*(100-99.2-.3)),"%\n")
        print(' ==========================================\n')
