window.js_init = init;
window.js_selectModel = selectModel;
...
function init() {
  ...
  // No longer calls showLog() directly, as selectModel will handle it.
}
...
async function selectModel() {
  ...
  modelDropdown.disabled = true; // Disable dropdown
  outputDiv.innerHTML = "Loading model configuration...";

  try {
    const selIndex = modelDropdown.selectedIndex;
    setCookie(nameSelIndex, selIndex, 1000);
    const folder = modelDropdown.options[selIndex].text;
    console.log("JS: Loading folder:", folder);

    showLog(); // Update log links

    const response = await fetch(`${folder}/config.txt`);
    if (!response.ok) {
      throw new Error(`Failed to fetch config.txt: ${response.statusText}`);
    }
    const text = await response.text();
    const features = text.trim().split(",");

    if (!features || features.length === 0 || (features.length === 1 && features[0] === '')) {
        throw new Error("Features list is empty or invalid.");
    }

    setCookie("features", features, 1000);
    console.log("JS: Features loaded:", features);
    ...
    console.error('JS Error in selectModel:', error);
    outputDiv.innerHTML = `<span style="color: red;">Error: ${error.message}</span>`;
    return false; // Indicate failure

  } finally {
    modelDropdown.disabled = false; // ALWAYS re-enable dropdown
  }
}
