...
def prePCA(learnFile):
    ...
    #************************************
    # PCA: Experimental test
    #************************************
    mu = A.mean(axis=0)
    U,s,V = np.linalg.svd(A - mu, full_matrices=False)
    Zpca = np.dot(A - mu, V.transpose())

    Rpca = np.dot(Zpca[:,:2], V[:2,:]) + mu    # reconstruction
    err = np.sum((A-Rpca)**2)/Rpca.shape[0]/Rpca.shape[1]
    print('\n PCA reconstruction error with ' + str(numPCA) + ' PCs: ' + str(round(err,3)) +'\n');
    ...
    m = keras.Sequential()
    print(A.shape)
    m.add(keras.layers.Dense(3,  activation='elu', input_shape=(5,)))
    m.add(keras.layers.Dense(2,  activation='elu'))
    m.add(keras.layers.Dense(1,    activation='linear', name="bottleneck"))
    m.add(keras.layers.Dense(2,  activation='elu'))
    m.add(keras.layers.Dense(3,  activation='elu'))
    m.add(keras.layers.Dense(4,  activation='sigmoid'))
    m.compile(loss='mean_squared_error', optimizer = keras.optimizers.Adam())
    history = m.fit(A, A, batch_size=8, epochs=10, verbose=1)
