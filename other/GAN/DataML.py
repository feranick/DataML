...
'''
******************************************************************
* DataML Classifier and Regressor - Generator Adversarial Network
* 20210609a - experimental
* Uses: TensorFlow
...
def train(learnFile, testFile, normFile):
    ...
    ### Build model
    #************************************
    def get_Gmodel(learnRate = dP.l_rate, decay = dP.l_rdecay, layers = [e for e in reversed(dP.HL)], l2 = dP.l2, dropout = dP.drop):
    ...
    ### Train
    #************************************
    discriminator =  get_DM(get_model())
    adversarial = get_AM(get_Gmodel(), get_model())
    generator = get_Gmodel()

    for i in range(dP.epochs):
        data_train = A[np.random.randint(0,
                A.shape[0], size=dP.batch_size)]
                
        rand = np.random.uniform(0, 1.0, size=[A.shape[0], 9])
        noise = np.multiply(A,rand)
        
        data_fake = generator.predict(noise)
        x = np.concatenate((data_train, data_fake))
        #y = np.ones([2*dP.batch_size, 1])
        y1 = np.ones([dP.batch_size, 1])
        y = np.concatenate((Cl2.reshape(-1,1), y1))
        y[dP.batch_size:, :] = 0
    
        d_loss = discriminator.train_on_batch(x, y)
        dv_loss = discriminator.test_on_batch(A_test)
        ...
        av_loss = adversarial.test_on_batch(A_test)
        
        log_mesg = "%d: [D loss: %f, acc: %f, v_loss: %f, v_acc: %f]" % (i, d_loss[0], d_loss[1], dv_loss[0], dv_loss[1])
        log_mesg = "\n %s  [A loss: %f, acc: %f, v_loss: %f, v_acc: %f]" % (log_mesg, a_loss[0], a_loss[1], av_loss[0], av_loss[1])
        print(log_mesg)
    ...
    if dP.regressor:
        ...
        print('  ==========================================================')
        print("  \033[1mLossD\033[0m - Average: {0:.4f}; Min: {1:.4f}; Last: {2:.4f}".format(np.average(d_loss[0]), np.amin(d_loss[0]), d_loss[0]))
        print("  \033[1mLossA\033[0m - Average: {0:.4f}; Min: {1:.4f}; Last: {2:.4f}".format(np.average(a_loss[0]), np.amin(a_loss[0]), a_loss[0]))
        print("  \033[1mMean Abs Err D\033[0m - Average: {0:.4f}; Min: {1:.4f}; Last: {2:.4f}".format(np.average(d_loss[1]), np.amin(d_loss[1]), d_loss[1]))
        print("  \033[1mMean Abs Err A\033[0m - Average: {0:.4f}; Min: {1:.4f}; Last: {2:.4f}".format(np.average(a_loss[1]), np.amin(a_loss[1]), a_loss[1]))
        print('\n  ==========================================================')
        print('  \033[1m MLP - Regressor \033[0m - Validation Summary')
        print('  ========================================================')
        print("  \033[1mLossD\033[0m - Average: {0:.4f}; Min: {1:.4f}; Last: {2:.4f}".format(np.average(dv_loss[0]), np.amin(dv_loss[0]), dv_loss[0]))
        print("  \033[1mLossA\033[0m - Average: {0:.4f}; Min: {1:.4f}; Last: {2:.4f}".format(np.average(av_loss[0]), np.amin(av_loss[0]), av_loss[0]))
        print("  \033[1mMean Abs Err D\033[0m - Average: {0:.4f}; Min: {1:.4f}; Last: {2:.4f}".format(np.average(dv_loss[1]), np.amin(dv_loss[1]), dv_loss[1]))
        print("  \033[1mMean Abs Err A\033[0m - Average: {0:.4f}; Min: {1:.4f}; Last: {2:.4f}".format(np.average(av_loss[1]), np.amin(av_loss[1]), av_loss[1]))
        #if dP.saveBestModel:
