...
# module. It MUST be imported before `import keras` below: importing it performs
# device SELECTION (sets CUDA_VISIBLE_DEVICES etc.) before the Keras backend
# initializes CUDA. configureDevices() (called in main()) then applies the
# backend-specific memory tuning.
...
def generate(learnFile):
    ...
    if not os.path.exists(dP.modelName):
        ...
        print("  Train one first:  DataML_VAE -t <learningFile>\n")
        return 1
    ...
    if dP.removeSpurious:
        ...
        if newA.shape[0] == 0 or frac_cells > 0.90:
            ...
            print("  removeSpurious = False to inspect the raw generated/n"
                  " values.")
            return 1
    ...
    print('\n  ==============================================================')
    print('   \033[1mGenerated VAE data\033[0m')
    print('  ==============================================================')
    ...
    if dP.plotAugmData:
        plotData(dP, orig_physical_A, newA, True, False,
                 "VAE generated data", rootFile + "_plots.pdf")
...
def generateData(dP, autoencoder, A, M, norm):
    ...
    # ---------------------------------------------------------------
    # Discrete columns cannot be reliably reproduced by the VAE: the
    # reconstruction drifts off the real steps (worse than a plain AE,
    # since the Sampling layer injects stochastic noise even at
    # inference), and the drift is then purged downstream by
    # snap_discrete_features. Since the seeds already carry the exact,
    ...
    # consistent with the rest of normVae.
    # ---------------------------------------------------------------
    for i in range(1, A.shape[1]):
        unique_vals = np.unique(A[:, i])
        if len(unique_vals) <= dP.discreteThreshold:
