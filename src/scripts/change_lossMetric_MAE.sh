...
find "$folder_path" -type f -name "$filename" | while read -r file; do
    ...
    if grep -q "lossMetric = mean_squared_error" "$file"; then
        echo "  Found 'lossMetric = mean_squared_error'. Replacing it with 'lossMetric = mean_absolute_error'."
        ...
        sed -i '' 's/lossMetric = mean_squared_error/lossMetric = mean_absolute_error/g' "$file"
        echo "  Replacement complete for $file"
    else
        echo "  'lossMetric = mean_squared_error' not found in $file. Skipping replacement."
    fi
done
