...
find "$folder_path" -type f -name "$filename" | while read -r file; do
    ...
    if grep -q "plotSpecificColors = False" "$file"; then
        echo "  Found 'plotSpecificColors = False'. Replacing it with 'plotSpecificColors = True'."
        ...
        sed -i '' 's/plotSpecificColors = False/plotSpecificColors = True/g' "$file"
        echo "  Replacement complete for $file"
    else
        echo "  'plotSpecificColors = False' not found in $file. Skipping replacement."
    fi
done
